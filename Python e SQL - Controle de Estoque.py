...
# Verificar se o insumo existe no banco de dados para fazer uma função
# Fazer Clean Code
...
def adicionar_insumo():
    cursor.execute(f"""
        ...
        VALUES
        ('{nome_insumo.get()}',{qtde_insumo},'{data_insumo.get()}',{lote_insumo.get()})
""")
    ...
    caixa_texto.insert("1.0", f"Insumo {nome_insumo} adicionado com sucesso!")
    
def deletar_insumo():
    if len(nome_insumo.get()) < 2:
        caixa_texto.delete("1.0", END)
        caixa_texto.insert("1.0", "nome insumo inválido")
        return
    
    #VERIFICAR SE TEM O PRODUTO NO DB antes de deletar
    cursor.execute(f'''
        DELETE FROM Estoque
...
def consumir_insumo():
    ...
    if len(nome_insumo.get()) < 2 or len(lote_insumo.get()) < 1:
        caixa_texto.delete("1.0", END)
        caixa_texto.insert("1.0", "lote do insumo inválido")
        return
    ...
    # escrever na caixa de texto
    caixa_texto.insert("1.0", f"Insumo {nome_insumo} foi consulmido em {qtde_insumo} unidade(s)")
...
def visualizar_insumo():
    ...
    for id_product, name, qtd, validade, lote in values:
        texto = texto + f"""
        -----
        Produto: {name}
        Id: {id_product}
